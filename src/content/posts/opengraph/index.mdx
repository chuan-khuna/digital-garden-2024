...
### 3. OG Image Templates (`src/components/og/`)
...
### 4. Dynamic Route Handlers (`src/pages/og/`)

Astro API routes that generate OG images on-demand (at build time):

#### Posts Route (`og/posts/[...slug].png.ts`)

Generates OG images for blog posts from the `posts` collection:

```typescript
// URL pattern: /og/posts/my-post-slug.png
...
Posts can specify their OG style in frontmatter:
...
#### Pages Route (`og/pages/[...slug].png.ts`)

Generates OG images for static pages using the `ogImages` content collection:

```typescript
// URL pattern: /og/pages/about.png

const ogImages = await getCollection('ogImages')
```

**Configuration file:** `src/content/og-images.json`
...
- [[seo]] - Related SEO optimization techniques
- [[astro-content-collections]] - Content collection architecture
